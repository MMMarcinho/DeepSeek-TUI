...
/// Vision model configuration for dedicated image and visual processing.
...
pub struct VisionModelConfig {
    ...
    /// API key for the vision model. If not specified, inherits from main config.
    #[serde(default)]
    pub api_key: Option<String>,
    /// Base URL for the vision model API. If not specified, inherits from main config.
    #[serde(default)]
    pub base_url: Option<String>,
    /// Maximum tokens for vision model responses
...
pub struct Config {
    ...
    /// Vision model configuration for image and visual processing tasks.
    /// When configured, all image-related operations will use this dedicated
    /// vision model with independent session management (subagent mode).
    #[serde(default)]
...
impl Config {
    ...
    /// Return the configured vision model configuration, if any.
    #[must_use]
    pub fn vision_model_config(&self) -> Option<VisionModelConfig> {
        self.vision_model.clone()
    }

    /// Check if vision model is enabled and properly configured.
    #[must_use]
    pub fn vision_model_enabled(&self) -> bool {
        self.vision_model.is_some()
    }

    /// Resolve the effective vision model configuration with inherited values.
    /// Inherits provider, api_key, and base_url from main config if not specified.
    #[must_use]
    pub fn resolve_vision_model_config(&self) -> Option<VisionModelConfig> {
        let mut config = self.vision_model.clone()?;

        // Inherit provider from main config if not specified
        ...
        // Inherit base URL from main config if not specified
